...
namespace hh
{
  ...
  public:
    ...
    interrupt get_exit_interrupt();
    exception_error_code exit_interruption_error_code() const noexcept;
    vmx::interrupt_info exit_interruption_info() const noexcept;
    ...
    x86::msr::efer_t host_efer() const noexcept;
    void host_efer(x86::msr::efer_t efer) noexcept;

    /* VMENTRY state */

    exception_error_code entry_interruption_error_code() const noexcept;
    ...
    void set_nmi_window_exiting(bool set);
    void inject_interrupt(interrupt input_interrupt);
    void entry_interruption_info(vmx::interrupt_info info);
    ...
    x86::msr::efer_t guest_efer() const noexcept;
    void guest_efer(x86::msr::efer_t efer) noexcept;

    uint64_t guest_activity_state() const noexcept;
    void guest_activity_state(uint64_t) noexcept;

    void switch_guest_paging_mode(x86::cr0_t cr0) noexcept;
    ...
    hook::hook_info** mtf_restore_point() noexcept;
  ...
  private:
    ...
    void save_state_and_start_virtualization();
    ...
    // The VT-x requires the guest task segment register to be configured correctly
    // and the UEFI platform typically does not (ie, TR being zero). So we just setup
    // our GDT with TR.
    void setup_host_gdt();

  public:
    explicit vcpu(std::shared_ptr<hv_event_handlers::vmexit_handler> exit_handler);
