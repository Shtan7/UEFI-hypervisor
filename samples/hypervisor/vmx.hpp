...
#define GUEST_SEG_BASE(sel) (GUEST_ES_BASE + (sel) * 2) /* ES ... GS */
    ...
    struct virtual_machihe_state_t
    {
      ...
      uint64_t vmxon_region_physical_address;
      uint8_t* vmxon_region_virtual_address;
      uint64_t vmcs_region_physical_address;
      uint8_t* vmcs_region_virtual_address;
      uint8_t* vmm_stack;	// Stack for VMM in VM-Exit State
      uint8_t* msr_bitmap_virtual_address;
      uint64_t msr_bitmap_physical_address;
      vmxoff_state_t vmxoff_state; // Shows the vmxoff state of the guest
      hook::hook_info* mtf_ept_hook_restore_point; // It shows the detail of the hooked paged that should be restore in MTF vm-exit
      x86::gdtr_t original_gdtr; // Ptr to original guest GDT
      x86::gdt_entry_t host_guest_gdt_entries[16]; // New GDT for guest and host
      x86::gdtr_t host_guest_gdtr;
      x86::task_state_segment host_guest_tss; // Properly setuped TSS for VMX
    };

    // Which exceptions we should intercept
    ...
    enum class guest_activity_state
    {
      active,
      halt,
      shutdown,
    ...
    // Adjust cr4 to correct state for VMX
    inline x86::cr4_t adjust_cr4(x86::cr4_t old_value)
    {
      old_value.all &= x86::msr::read<x86::msr::vmx_cr4_fixed1>();
      old_value.all |= x86::msr::read<x86::msr::vmx_cr4_fixed0>();

      return old_value;
    }

    // Adjust cr0 to correct state for VMX
    inline x86::cr0_t adjust_cr0(x86::cr0_t old_value)
    {
      old_value.all &= x86::msr::read<x86::msr::vmx_cr0_fixed1>();
      old_value.all |= x86::msr::read<x86::msr::vmx_cr0_fixed0>();

      return old_value;
    }

    inline x86::cr4_t adjust_guest_cr4(x86::cr4_t old_value)
    {
      return adjust_cr4(old_value);
    }

    inline x86::cr0_t adjust_guest_cr0(x86::cr0_t old_value, x86::msr::vmx_procbased_ctls2_t secondary_proc_based_controls)
    {
      x86::cr0_t new_value = adjust_cr0(old_value);

      if (secondary_proc_based_controls.flags.unrestricted_guest != false)
      {
        new_value.flags.protection_enable &= old_value.flags.protection_enable;
        new_value.flags.paging_enable &= old_value.flags.paging_enable;
      }

      return new_value;
    }

    // Adjust controls to correct state for VMX
    inline uint32_t adjust_controls(uint32_t ctl, uint32_t msr) noexcept
    {
